<template>
  <div v-show="Object.keys(allTxs).length > 0 || Object.keys(jobs('white')).length > 0" class="txs">
    ...
    <Job v-for="job in jobs('white')" :key="job.id" :job="job" />
    <template v-for="tx in filteredTxs">
      <EncryptedTx v-if="tx.isEncrypted" :key="tx.txHash" :tx="tx" />
      <Tx v-else :key="tx.txHash" :tx="tx" />
    </template>
    <div v-show="filteredTxs.length === 0 && jobs('white').length === 0" class="box box-tx is-white">
